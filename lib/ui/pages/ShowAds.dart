...
//import 'package:souq_alfurat_web/ui/Report.dart';
//import 'package:souq_alfurat_web/ui/private_chat.dart';
//import 'package:url_launcher/url_launcher.dart';
...
class _ShowAdState extends State<ShowAd> {
  ...
  getDocumentValue() async {
    ...
    adImagesUrl = documentsAds['imagesUrl'];
    setState(() {
      showSlider = true;
      showBody = true;
      isRequest = documentsAds['isRequest'];
    });
  ...
  //     'title': "${documentsUser['name']} علق على  ${AdsN}",
  //     "Mess": "${Messgetext}"
  //   };
  ...
  Future<void> callBack() async {
    ...
    if (messageController.text.length > 0) {
      ...
      documentRef = Firestore.instance
          .collection('users')
          .document(documentsAds['uid']);
      documentsUser = await documentRef.get();
      print("token" + documentsUser['token']);
      print(documentsAds['uid']);
      print(documentsUser.documentID);
      if (documentsAds['uid'] != currentUserId) {
        //makePostRequest(documentsUser['token'], documentsAds['name']);
      }
